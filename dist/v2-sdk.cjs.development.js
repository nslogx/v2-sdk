...
var Pair = /*#__PURE__*/ (function() {
  ...
  _proto.getOutputAmount = function getOutputAmount(inputAmount) {
    ...
    var outputAmount = sdkCore.CurrencyAmount.fromRawAmount(
  ...
  _proto.getInputAmount = function getInputAmount(outputAmount) {
    ...
    var inputAmount = sdkCore.CurrencyAmount.fromRawAmount(
  ...
  _proto.getLiquidityMinted = function getLiquidityMinted(totalSupply, tokenAmountA, tokenAmountB) {
    ...
    if (JSBI.equal(totalSupply.quotient, ZERO)) {
      liquidity = JSBI.subtract(
        sdkCore.sqrt(JSBI.multiply(tokenAmounts[0].quotient, tokenAmounts[1].quotient)),
        MINIMUM_LIQUIDITY
      )
    } else {
  ...
  _proto.getLiquidityValue = function getLiquidityValue(token, totalSupply, liquidity, feeOn, kLast) {
    ...
    return sdkCore.CurrencyAmount.fromRawAmount(
      token,
  ...
  _createClass(Route, [
    {
      ...
      get: function get() {
        ...
        for (
          var _iterator2 = _createForOfIteratorHelperLoose(this.pairs.entries()), _step2;
...
var Trade = /*#__PURE__*/ (function() {
  function Trade(route, amount, tradeType) {
    ...
    if (tradeType === sdkCore.TradeType.EXACT_INPUT) {
      ...
      this.outputAmount = sdkCore.CurrencyAmount.fromFractionalAmount(
        route.output,
      ...
      this.inputAmount = sdkCore.CurrencyAmount.fromFractionalAmount(
        route.input,
    ...
    this.executionPrice = new sdkCore.Price(
  ...
  Trade.bestTradeExactIn = function bestTradeExactIn(
    ...
    for (var i = 0; i < pairs.length; i++) {
      ...
      if (amountOut.currency.equals(tokenOut)) {
        sdkCore.sortedInsert(
          bestTrades,
          new Trade(
            new Route([].concat(currentPairs, [pair]), currencyAmountIn.currency, currencyOut),
            currencyAmountIn,
    ...
    for (var i = 0; i < pairs.length; i++) {
      ...
      if (amountIn.currency.equals(tokenIn)) {
        sdkCore.sortedInsert(
          bestTrades,
          new Trade(
            new Route([pair].concat(currentPairs), currencyIn, currencyAmountOut.currency),
            currencyAmountOut,
